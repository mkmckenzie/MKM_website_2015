    ...
    this.each(function(){
      var $dom = $(this);
        ...
        if (options !== undefined && (typeof(options) == "boolean" || typeof(options) == "string")) { // Force flip the DOM
          ...
          if (settings.forceHeight) {faces.outerHeight($dom.height());} //else if (settings.autoSize) {faces.css({'height': 'auto'});}
          if (settings.forceWidth) {faces.outerWidth($dom.width());} //else if (settings.autoSize) {faces.css({'width': 'auto'});}
          faces.css({
